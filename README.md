...
```
harness-engineering/
├── harness-client/          # Client SDK - 服务接口定义和 DTO
│   ├── src/main/java/
...
│   └── pom.xml
│
├── harness-adapter/         # Adapter 层 - 适配层（Controller）
│   ├── src/main/java/
│   │   └── com.harness.engineering.adapter/
│   │       └── web/         # Web 控制器
│   └── pom.xml
│
├── harness-app/             # App 层 - 应用层（业务协调）
│   ├── src/main/java/
...
│   └── pom.xml
│
├── harness-domain/          # Domain 层 - 领域层（核心业务逻辑）
│   ├── src/main/java/
...
│   └── pom.xml
│
├── harness-infrastructure/  # Infra 层 - 基础设施层
│   ├── src/main/java/
...
1. **定义 DTO**（harness-client/dto）
   - 创建 Command/Query 对象
   - 创建 Response DTO

2. **定义接口**（harness-client/api）
   - 在 Service 接口中添加方法定义

3. **实现领域模型**（harness-domain/model）
   - 定义领域实体
   - 实现领域行为

4. **定义网关接口**（harness-domain/gateway）
   - 定义数据操作接口

5. **实现 App 层**（harness-app/executor）
   - 实现 Command 执行器
   - 实现 Query 执行器

6. **实现 Service**（harness-app/service）
   - 实现 Client 层定义的接口

7. **实现网关**（harness-infrastructure/gatewayimpl）
   - 实现 Domain 层定义的网关接口

8. **实现 Controller**（harness-adapter/web）
   - 定义 RESTful 接口
   - 调用 App 层服务
